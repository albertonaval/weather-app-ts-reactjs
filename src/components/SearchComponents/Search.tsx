...
const Search = ({term, options, onOptionSelect, onInputChange, onSubmit}: Props): JSX.Element => {
    ...
    <main className="flex justify-center items-center bg-gradient-to-br from-sky-400 via-rose-400 to-lime-400 h-[100vh] w-full">
      <section className="w-full md:max-w-[500px] p-4 flex flex-col text-center items-center justify-center md:px-10 lg:p-24 h-full lg:h-[500px] bg-white bg-opacity-20 backdrop-blur-lg drop-shadow-lg rounded text-zinc-700">
        ...
        <p className="text-sm mt-2">
          Enter below a place you want to know the weather of and select an option from dropdow.
        </p>
          ...
          <ul className="absolute top-9 bg-white ml-1 rounded-b-md">
            {options.map((option: optionType, index: number) => (
              <li key={option.name + '' + index}>
                <button className="text-left text-sm w-full hover:bg-zinc-500 hover:text-white px-2 py-1 cursor-pointer" onClick={() => onOptionSelect(option)}>
